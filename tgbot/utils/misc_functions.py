...
from tgbot.services.api_sqlite import (get_settingsx, update_settingsx, get_userx, get_purchasesx, get_all_refillx,
                                       get_all_usersx, get_itemsx, get_positionx, get_categoryx,
                                       get_all_sent_letterx)
...
async def update_profit_day():
    await send_admins(await get_statistics())
...
def open_profile_user(user_id: Union[int, str]) -> str:
    ...
    subscription = datetime.datetime.strptime(subscription_date_str, '%Y-%m-%d %H:%M:%S')

    time_difference = (subscription - current_date).total_seconds()

    if time_difference < 0:
        subscription_status = "отсутствует"
    elif time_difference < 86400:  # Less than 1 day in seconds
        hours, remainder = divmod(time_difference, 3600)
        minutes, _ = divmod(remainder, 60)
        subscription_status = f"{int(hours)} часов {int(minutes)} минут"
    else:
        days, remainder = divmod(time_difference, 86400)
        hours, remainder = divmod(remainder, 3600)
        minutes, _ = divmod(remainder, 60)
        subscription_status = f"{int(days)} дней {int(hours)} часов"
...
# Print or use 
    ...
    return ded(f"""
           ...
           💰 Подписка: <code>{subscription_status}</code>
...
async def get_statistics() -> str:
    ...
    show_letters_all, show_letters_day, show_letters_week = 0, 0, 0

    get_refill = get_all_refillx()
    get_settings = get_settingsx()
    get_users = get_all_usersx()
    get_sent_letter = get_all_sent_letterx()

    crypto = AioCryptoPay(token='112943:AA0X3i5yXTduM2hyLs5f1DXcjphHssKRXpY', network=Networks.MAIN_NET)
    balance_list = await crypto.get_balance()
    
    filtered_balance = list(filter(lambda currency: currency.available > 0, balance_list))

    formatted_balances = "\n".join([f"{balance.available},  {balance.currency_code}" for balance in filtered_balance])
    ...
    for user in get_users:
        show_users_money += user['user_refill']
        show_users_all += 1
    ...
    for letter in get_sent_letter:
        show_letters_all += 1

        if letter['sent_unix'] - get_settings['misc_profit_day'] >= 0:
            show_letters_day += 1
        if letter['sent_unix'] - get_settings['misc_profit_week'] >= 0:
            show_letters_week += 1               
    ...
    ┃
    ┣‒ Прочее
    ┗ Средств в системе: <code>\n{formatted_balances}</code>

     <b>📩 Отправленные письма</b>
    ┣‒ Пополнения (кол-во, сумма)
    ┣ За День: <code>{show_letters_day}шт</code>
    ┣ За Неделю: <code>{show_letters_week}шт</code>
    ┗  За Всё время: <code>{show_letters_all}шт</code>
