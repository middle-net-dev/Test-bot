...
# Проверка работоспособности QIWI
@dp.message_handler(IsAdmin(), text="🤖 Неоплаченные счета Cryptobot ♻", state="*")
async def payment_qiwi_check(message: Message, state: FSMContext):
    await state.finish()
    ...
    unpaid_invoices = list(filter(lambda invoice: invoice.status == 'active', invoices))

    if not unpaid_invoices:
        await message.answer(f"<b>Неоплаченых счетов - нет :)</b>")
    else:
        formatted_invoices = "\n".join([f"InvoiceId: {invoice.invoice_id}, Amount: {invoice.amount}, Currency: {invoice.asset}" for invoice in unpaid_invoices])

        await message.answer(f"<b>{formatted_invoices}</b>")
...
# Баланс криптобот
@dp.message_handler(IsAdmin(), text="🤖 Баланс Cryptobot 👁", state="*")
async def payment_qiwi_balance(message: Message, state: FSMContext):
    await state.finish()
    ...
    formatted_balances = "\n".join([f"Currency: {balance.currency_code}, Available: {balance.available}" for balance in filtered_balance])

    await message.answer(f"<b>{formatted_balances}</b>")
