...
async def functions_profile_purchases(call: CallbackQuery, state: FSMContext):
    ...
    last_refills = last_refillsx(user_id, 10)
...
@dp.callback_query_handler(IsAdmin(), text_startswith="admin_user_subscription_add", state="*")
async def functions_profile_balance_add(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "<b>💰 Введите количество дней для выдачи баланса</b>",
...
async def functions_profile_balance_add_get(message: Message, state: FSMContext):
    ...
    if(time_difference > 0):
        update_userx(user_id, user_subscription=subscription + timedelta(to_number(message.text)))
    else:
         update_userx(user_id, user_subscription=current_date + timedelta(to_number(message.text)))

    await message.answer(
        f"<b>✅ Пользователю <a href='tg://user?id={get_user['user_id']}'>{get_user['user_name']}</a> "
        f"выдано <code>{message.text} дней</code></b>"
    )

    await message.bot.send_message(user_id, f"<b>💰 Вам было выдано <code>{message.text} дней</code></b>")
    await message.answer(
...
@dp.callback_query_handler(IsAdmin(), text_startswith="admin_user_subscription_set", state="*")
...
async def functions_profile_balance_set_get(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"<b>✅ Пользователю <a href='tg://user?id={get_user['user_id']}'>{get_user['user_name']}</a> "
        f"изменён баланс на <code>{message.text} дней</code></b>")
