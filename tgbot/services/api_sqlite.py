...
def add_userx(user_id, user_login, user_name):
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        con.execute("INSERT INTO storage_users "
                    "(user_id, user_login, user_name, user_subscription, user_refill, user_date, user_unix) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [user_id, user_login, user_name, '2023-08-01 00:00:00', 0, get_date(), get_unix()])
...
def get_userx(**kwargs):
 ...
 # Добавление инфо про отправленное письмо
def add_sent_letter_infox(user_id, country, service, link, email, sent_date, sent_unix):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO sent_letters "
                    "(user_id, country, service, link, email, sent_date, sent_unix) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [user_id, country, service, link, email, sent_date, sent_unix])        

# Добавление данных для отправки писем
def add_data_to_sendx(user_id):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO data_to_send "
                    "(user_id) "
                    "VALUES (?)",
                    [user_id])       

# Изменение данных на отправку
def update_data_to_sendx(user_id, **kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "UPDATE data_to_send SET"
        sql, parameters = update_format(sql, kwargs)
        parameters.append(user_id)
        con.execute(sql + "WHERE user_id = ?", parameters)   

# Получение данных для отправки
def get_data_go_sendx(user_id, **kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "SELECT * FROM data_to_send"
        sql, parameters = update_format(sql, kwargs)
        parameters.append(user_id)
        return con.execute(sql + "WHERE user_id = ?", parameters).fetchall()
...
def add_refillx(user_id, user_login, user_name, refill_receipt_id, refill_amount, refill_currency,
                refill_comment, refill_way, refill_date, refill_unix):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO storage_refill "
                    "(user_id, user_login, user_name, refill_receipt_id, refill_amount, refill_currency, refill_comment, refill_way, refill_date, refill_unix) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [user_id, user_login, user_name, refill_receipt_id, refill_amount, refill_currency, refill_comment,
...
def get_all_sent_letterx():
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "SELECT * FROM sent_letters"
        return con.execute(sql).fetchall()
...
def last_refillsx(user_id, count):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"SELECT * FROM storage_refill WHERE user_id = ? ORDER BY increment DESC LIMIT {count}"
        return con.execute(sql, [user_id]).fetchall()
    ...
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        if len(con.execute("PRAGMA table_info(storage_refill)").fetchall()) == 11:
            ...
            con.execute("CREATE TABLE storage_refill("
                        ...
                        "user_login TEXT,"
                        "user_name TEXT,"
                        "refill_receipt_id INTEGER,"
                        "refill_amount INTEGER,"
                        "refill_currency TEXT,"
                        "refill_comment TEXT,"
                        "refill_way TEXT,"
        ...
        # # Создание БД с хранением покупок
        if len(con.execute("PRAGMA table_info(data_to_send)").fetchall()) == 6:
            print("DB was found(9/9)")
        else:
            con.execute("CREATE TABLE data_to_send("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "user_id INTEGER,"
        ...
        # # Создание БД с хранением отправленных писем
        if len(con.execute("PRAGMA table_info(sent_letters)").fetchall()) == 8:
            print("DB was found(10/10)")
        else:
            con.execute("CREATE TABLE sent_letters("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "user_id INTEGER,"
                        "country TEXT DEFAULT NULL,"
                        "service TEXT DEFAULT NULL,"
